"""
VStream-QA Ego4D 数据集视频问答
并行化版本：使用 asyncio 异步并发请求加速处理

支持功能：
1. 读取 test_qa_ego4d.json 问题文件
2. 根据 video_id 找到对应的帧文件夹
3. 读取从 start_time 到 end_time 对应的所有帧（基于秒数）
4. 使用 vLLM API 进行开放式 VQA 问答
5. 使用 GPT 模型对回答进行评分
6. 按 answer_type 分类输出结果
"""
...
# 默认文件路径
DEFAULT_VIDEO_DIR = "/data/tempuser2/MMAgent/VStream-QA/ego4d_frames"
DEFAULT_JSON_PATH = "/data/tempuser2/MMAgent/MM-Mem/Baseline/VStream-QA/Dataset/test_qa_ego4dPPP.json"
...
# Ego4D 帧率配置（用于根据秒数计算帧编号）
FRAME_RATE = 1  # 每秒对应的帧数（如果帧是按秒提取的，则为1）
...
def parse_frame_number(frame_name: str) -> int:
    ...
    Returns:
        int: 帧编号，解析失败返回 -1
    """
    match = re.match(r'(\d+)\.jpg', frame_name)
    if match:
        return int(match.group(1))
    return -1


def get_frames_in_range_ego4d(video_dir: str, video_id: str, start_time: int, end_time: int, 
                               use_start_time: bool = True, frame_rate: int = 1) -> List[str]:
    """
    获取 Ego4D 数据集指定时间范围内的所有帧文件路径。
    ...
    # 获取所有 jpg 文件（支持多种命名格式）
    all_frames = glob.glob(os.path.join(video_folder, "*.jpg"))
    ...
    def sort_key(path):
        ...
        return frame_num if frame_num >= 0 else float('inf')
    
    all_frames_sorted = sorted(all_frames, key=sort_key)
    
    # 根据时间范围筛选帧
    # 假设帧编号从1开始，对应第0秒开始的帧
    start_frame = (start_time * frame_rate) + 1 if use_start_time else 1
    end_frame = (end_time * frame_rate) + 1
...
async def ask_question_async(
    ...
    # 构建提示词
    prompt = (
        "Answer the following open-ended question based on the video.\n"
        f"Question: {question}\n"
    )
    ...
    for coro in asyncio.as_completed(async_tasks):
        ...
        if result.get('success'):
            print(f"[{completed:>{progress_width}}/{total_tasks}] ✓ "
                  ...
                  f"类型={result['answer_type'][:15]:<15} "
                  f"问题: {result['question'][:30]}...")
        else:
            print(f"[{completed:>{progress_width}}/{total_tasks}] ⚠ "
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="VStream-QA Ego4D 数据集视频问答测试工具",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
示例用法:
  python Async_API_answer_Ego4D_questions.py
  python Async_API_answer_Ego4D_questions.py -j ./test_qa.json -v ./Video/Ego_4D -c 4
        """
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="输出结果文件路径 (默认: 当前目录下的 ego4d_vqa_results.json)"
    )
    ...
    parser.add_argument(
        ...
        help="从 start_time 开始读取帧，而非从第一帧开始 (默认: True)"
    )
...
async def main_async():
    ...
    global GPT_API_KEY, GPT_API_BASE, GPT_ENGINE, IMAGE_SIZE, USE_START_TIME, FRAME_RATE
    ...
    output_path = args.output or os.path.join(os.getcwd(), 'ego4d_vqa_results.json')
    
    timer_start = time.time()
    
    print("=" * 70)
    print("VStream-QA Ego4D 数据集视频问答测试")
    print("=" * 70)
    ...
    for q in questions:
        ...
        q_start_time = q.get("start_time", 0)
        q_end_time = q.get("end_time", 0)
        ...
        # 获取帧文件（Ego4D 使用时间戳格式）
        frame_paths = get_frames_in_range_ego4d(
            video_dir, video_id, q_start_time, q_end_time, USE_START_TIME, FRAME_RATE
        )
